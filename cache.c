...
#if CMU_ENABLE_SAMPLE_DEBUGGING
  extern int sb_size; //For debugging, to assert if there is a ridiculous cache miss latency
  extern int mem_lat[2];
  extern FILE* fSampleMisses;
# if (CMU_ENABLE_DTLB_TRACE)
    extern FILE* fSampleDTLB;
# endif //(CMU_ENABLE_DTLB_TRACE)
  extern struct cache_t * cache_dl2;
  extern struct cache_t * dtlb;
  extern struct cache_t * itlb;
  extern int miss_trace_enable;
  extern counter_t sim_pop_insn;
  extern unsigned long icount_low, icount_high;
#endif //CMU_ENABLE_SAMPLE_DEBUGGING
             ...
             unsigned int hit_latency)  /* latency in cycles for a hit */
{
  ...
  /* initialize the mshrs: mshr entry 0 is always necessary */
  ...
  /* pipelined L2 cache */
  cp->nlev_issuelat = 1;	/* one access per cycle for next level cache */
  ...
  cp->wrong_path_hits= 0;
  cp->wrong_path_misses = 0;
  cp->wrong_path_replacements = 0;
  cp->wrong_path_writebacks = 0;
  ...
  cp->mshr_misses = 0;
  cp->mshr_full = 0;
  cp->mshr_full_stall = 0;
  cp->mshr_target_full = 0;
  cp->mshr_target_full_stall = 0;
...
cache_create(char *name,		/* name of the cache */
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  return cache_create_mshr(name, nsets, bsize, balloc, usize, assoc, 0, 0,
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  sprintf(buf, "%s.wp_accesses", name);
  sprintf(buf1, "%s.wp_hits + %s.wp_misses", name, name);
  stat_reg_formula(sdb, buf, "total number of wp accesses", buf1, "%12.0f");
  sprintf(buf, "%s.wp_hits", name);
  stat_reg_counter(sdb, buf, "total number of wp hits", &cp->wrong_path_hits, 0, NULL);
  sprintf(buf, "%s.wp_misses", name);
  stat_reg_counter(sdb, buf, "total number of wp misses", &cp->wrong_path_misses, 0, NULL);
  sprintf(buf, "%s.wp_replacements", name);
  stat_reg_counter(sdb, buf, "total number of wp replacements",
		 &cp->wrong_path_replacements, 0, NULL);
  sprintf(buf, "%s.wp_writebacks", name);
  stat_reg_counter(sdb, buf, "total number of wp writebacks",
		 &cp->wrong_path_writebacks, 0, NULL);

  sprintf(buf, "%s.store_misses", name);
  stat_reg_counter(sdb, buf, "total number of store misses", &cp->store_misses, 0, NULL);
  ...
  sprintf(buf, "%s.wp_access_pct", name);
  sprintf(buf1, "%s.wp_accesses / %s.accesses", name, name);
  stat_reg_formula(sdb, buf, "wp access pct (i.e., wp_accesses/accesses)", buf1, NULL);
  sprintf(buf, "%s.wp_hit_pct", name);
  sprintf(buf1, "%s.wp_hits / %s.hits", name, name);
  stat_reg_formula(sdb, buf, "wp access pct(i.e., wp_hits/hits)", buf1, NULL);
  sprintf(buf, "%s.wp_miss_pct", name);
  sprintf(buf1, "%s.wp_misses / %s.misses", name, name);
  stat_reg_formula(sdb, buf, "wp miss pct(i.e., wp_misses/misses)", buf1, NULL);
  sprintf(buf, "%s.wp_replacement_pct", name);
  sprintf(buf1, "%s.wp_replacements / %s.replacements", name, name);
  stat_reg_formula(sdb, buf, "wp replacement pct(i.e., wp_replacements/replacement)", buf1, NULL);
  sprintf(buf, "%s.wp_writeback_pct", name);
  sprintf(buf1, "%s.wp_writebacks / %s.writebacks", name, name);
  stat_reg_formula(sdb, buf, "wp writeback pct(i.e., wp_writebacks/writebacks)", buf1, NULL);
  ...
  if (cp->num_mshrs != -1) {
     ...
     sprintf(buf, "%s.mshr_full_stall", name);
     stat_reg_counter(sdb, buf, "total delay to to mshr-full events", &cp->mshr_full_stall, 0, NULL);

     sprintf(buf, "%s.mshr_full_rate", name);
     sprintf(buf1, "%s.mshr_full / %s.mshr_accesses", name, name);
     stat_reg_formula(sdb, buf, "mshr full rate (i.e., full/accesses)", buf1, NULL);

     sprintf(buf, "%s.mshr_target_full", name);
     stat_reg_counter(sdb, buf, "total number of mshr-target-full events", &cp->mshr_target_full, 0, NULL);
     sprintf(buf, "%s.mshr_target_full_stall", name);
     stat_reg_counter(sdb, buf, "total delay due to mshr-target-full events", &cp->mshr_target_full_stall, 0, NULL);

     sprintf(buf, "%s.mshr_target_full_rate", name);
     sprintf(buf1, "%s.mshr_target_full / %s.mshr_accesses", name, name);
     stat_reg_formula(sdb, buf, "mshr tgt full rate (i.e., target-full/accesses)", buf1, NULL);
	     ...
	     int stat_flags)
{
  ...
  //myfprintf(stderr, "%s CACHE ACCESS to %llx\n", cp->name, addr);
  ...
  if (CACHE_TAGSET(cp, addr) == cp->last_tagset && (cp->last_tagset != 0))
    {
  ...
  //myfprintf(stderr, "%s CACHE MISS to %llx\n", cp->name, addr);

  /* mshr: search for empty mshr */
...
#if CMU_ENABLE_SAMPLE_DEBUGGING
  if (miss_trace_enable) {
    if (cp == cache_dl2 || cp == itlb || cp == dtlb) {
      if (cp == cache_dl2) {
        fprintf(fSampleMisses, "DL2 ");
      } else if (cp == itlb ) {
        fprintf(fSampleMisses, "ITLB ");
      } else if (cp == dtlb ) {
        fprintf(fSampleMisses, "DTLB ");
      }
      if (stat_flags & WRONGPATH_FLAG) {
        fprintf(fSampleMisses, "wrong-path ");
      }
      if (stat_flags & STORE_FLAG) {
        fprintf(fSampleMisses, "store ");
      }
      fprintf(fSampleMisses, "Miss [%llx]\n",addr);
    }
  }
#endif //CMU_ENABLE_SAMPLE_DEBUGGING

  if (stat_flags & WRONGPATH_FLAG) {
    cp->wrong_path_misses++;
  }
  ...
  /* bus open time */
  cp->bus_open += BOUND_POS(now - cp->bus_free);

# if (CMU_ENABLE_SAMPLE_DEBUGGING && CMU_ENABLE_DTLB_TRACE)
    if (cp == dtlb) {
      fprintf(fSampleDTLB,"DTLB Miss [%llx] insn(%lld) ",CACHE_MK_BADDR(cp, tag, set), sim_pop_insn);
    }
# endif //(CMU_ENABLE_SAMPLE_DEBUGGING && CMU_ENABLE_DTLB_TRACE)
...
#     if (CMU_ENABLE_SAMPLE_DEBUGGING && CMU_ENABLE_DTLB_TRACE)
        if (cp == dtlb) {
          fprintf(fSampleDTLB,"replaces [%llx] ",CACHE_MK_BADDR(cp, repl->tag, set));
        }
#     endif //(CMU_ENABLE_SAMPLE_DEBUGGING && CMU_ENABLE_DTLB_TRACE)
      ...
      /* mshr: lat += BOUND_POS(cp->bus_free - (now + lat)); */

      /* track bus resource usage */
      /* mshr: cp->bus_free = MAX(cp->bus_free, (now + lat)) + 1; */

      if (repl->status & CACHE_BLK_DIRTY)
	{
	  /* pipelined L2: stall until the bus to next level of memory is available */
	  lat += BOUND_POS(cp->bus_free - (now + lat));

	  /* pipelined L2: track bus resource usage */
	  cp->bus_free = MAX(cp->bus_free, (now + lat)) + cp->nlev_issuelat;

	  /* write back the cache block */
	  cp->writebacks++;

      if (stat_flags & WRONGPATH_FLAG) {
        cp->wrong_path_writebacks++;
      }


	  lat += cp->blk_access_fn(Write,
				   CACHE_MK_BADDR(cp, repl->tag, set),
				   cp->bsize, repl, now+lat, stat_flags);
	}
    }
...
# endif //(CMU_ENABLE_SAMPLE_DEBUGGING && CMU_ENABLE_DTLB_TRACE)
  ...
  /* mshr: update mshr status */
  if ((now) && (cp->num_mshrs != -1)) {

     cp->mshr[hit_mshr].ready = repl->ready;
     cp->mshr[hit_mshr].tagset = baddr;
     cp->mshr[hit_mshr].status = repl->status;
...
# if (CMU_ENABLE_SAMPLE_DEBUGGING)
  /* warn if we see a memory latency much larger than sb_size * memory latency */
  if (lat > (sb_size + 5) * mem_lat[0]) {
     warn("Encountered a suspicious miss latency of %d", lat);
  }
# endif
 ...
 cache_hit: /* slow hit handler */
  ...
  if ((now) && (cp->num_mshrs != -1)) {
     ...
     if (blk->ready > now) {
        ...
        for (i = 0; i < cp->num_mshrs; i++) {
           if (cp->mshr[i].tagset == baddr && cp->mshr[i].ready > now) {
	...
	md_addr_t addr)                /* address of access */
{
	...
	/* check for a fast hit: access to same block */
	if (CACHE_TAGSET(cp, addr) == cp->last_tagset) { /* hit in the same block */
		blk = cp->last_blk;
		goto cache_fast_hit;
	}
...
void * l2_fast_access(
	...
	counter_t cnt)
{
	md_addr_t tag = CACHE_TAG(cache_dl2, addr);
	md_addr_t set = CACHE_SET(cache_dl2, addr);
	struct cache_blk_t *blk, *repl;


	/* check for a fast hit: access to same block */
	if (CACHE_TAGSET(cache_dl2, addr) == cache_dl2->last_tagset) { /* hit in the same block */
		blk = cache_dl2->last_blk;
		goto cache_fast_hit;
	}

	if (cache_dl2->hsize) {
		/* higly-associativity cache, access through the per-set hash tables */
		int hindex = CACHE_HASH(cache_dl2, tag);
		for (blk = cache_dl2->sets[set].hash[hindex]; blk; blk = blk->hash_next) {
			if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
				goto cache_hit;
		}
	} else {
		/* low-associativity cache, linear search the way list */
		for (blk = cache_dl2->sets[set].way_head; blk; blk = blk->way_next) {
			if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
				goto cache_hit;
		}
	}
	...
	/* select the appropriate block to replace, and re-link this entry to
	  the appropriate place in the way list */
		repl = cache_dl2->sets[set].way_tail;
		update_way_list(&cache_dl2->sets[set], repl, Head);

	/* remove this block from the hash bucket chain, if hash exists */
	if (cache_dl2->hsize)
		unlink_htab_ent(cache_dl2, &cache_dl2->sets[set], repl);

	/* blow away the last block to hit */
	cache_dl2->last_tagset = 0;
	cache_dl2->last_blk = NULL;
	...
	/* link this entry back into the hash table */
	if (cache_dl2->hsize)
		link_htab_ent(cache_dl2, &cache_dl2->sets[set], repl);
...
cache_hit: /* slow hit handler */
	...
	if (blk->way_prev) {
		/* move this block to head of the way (MRU) list */
		update_way_list(&cache_dl2->sets[set], blk, Head);
	}

	/* tag is unchanged, so hash links (if they exist) are still valid */

	/* record the last block to hit */
	cache_dl2->last_tagset = CACHE_TAGSET(cache_dl2, addr);
	cache_dl2->last_blk = blk;
	blk->ready = cnt;
	...
	counter_t cnt)
{
	...
	/* low-associativity cache, linear search the way list */
	for (blk = cache_dl1->sets[set].way_head; blk; blk = blk->way_next) {
		if (blk->tag == tag && (blk->status & CACHE_BLK_VALID)) {
        	/* update dirty status */
        	if (cmd == Write)
            	blk->status |= CACHE_BLK_DIRTY;
        	...
        	if (blk->way_prev) {
        		/* move this block to head of the way (MRU) list */
        		update_way_list(&cache_dl1->sets[set], blk, Head);
        	}
	...
	/* select the appropriate block to replace, and re-link this entry to
	  the appropriate place in the way list */
		repl = cache_dl1->sets[set].way_tail;
		update_way_list(&cache_dl1->sets[set], repl, Head);

	/* write back replaced block data */
	if ((repl->status & CACHE_BLK_VALID) && (repl->status & CACHE_BLK_DIRTY)) {
		l2_fast_access(Write, CACHE_MK_BADDR(cache_dl1, repl->tag, set), cnt);
	}
...
# if (CMU_ENABLE_SAMPLE_DEBUGGING && CMU_ENABLE_DTLB_TRACE)
    if (cp == dtlb) {
      long long icount = sim_pop_insn + ((long long)icount_low) + (((long long)icount_high) << 32LL);
      fprintf(fSampleDTLB,"DTLB Miss [%llx] insn(%lld) ",CACHE_MK_BADDR(cp, tag, set), icount);
      if (repl->status & CACHE_BLK_VALID) {
          fprintf(fSampleDTLB,"replaces [%llx] ",CACHE_MK_BADDR(cp, repl->tag, set));
      }
      fprintf(fSampleDTLB,"\n");
...
void 				/* latency of the flush operation */
cache_clear(struct cache_t *cp)			/* time of cache flush */
{
  int i;
  struct cache_blk_t *blk;
...
/* mshr: count mshrs in use */
int count_mshr(struct cache_t *cp, tick_t now)
{
  int i, count = 0;

  for (i = 0; i < cp->num_mshrs; i++) {
      if (cp->mshr[i].ready > now) count++; /*?????*/
  }
...
void cache_alloc_stats(struct cache_t *cp) {
  cp->backup = calloc(1, sizeof(struct cache_stats_t));
}

void cache_backup_stats(struct cache_t *cp) {
  cp->backup->hits = cp->hits;
  cp->backup->misses = cp->misses;
  cp->backup->replacements = cp->replacements;
  cp->backup->writebacks = cp->writebacks;
  cp->backup->invalidations = cp->invalidations;
  cp->backup->mshr_hits = cp->mshr_hits;
  cp->backup->mshr_misses = cp->mshr_misses;
  cp->backup->mshr_full = cp->mshr_full;
  cp->backup->mshr_target_full = cp->mshr_target_full;
  cp->backup->wrong_path_misses =  cp->wrong_path_misses;
  cp->backup->wrong_path_hits =  cp->wrong_path_hits;
  cp->backup->wrong_path_writebacks =  cp->wrong_path_writebacks ;
  cp->backup->wrong_path_replacements =  cp->wrong_path_replacements;
  cp->backup->store_misses =  cp->store_misses;
}

void cache_restore_stats(struct cache_t *cp) {
  cp->hits = cp->backup->hits;
  cp->misses = cp->backup->misses;
  cp->replacements = cp->backup->replacements;
  cp->writebacks = cp->backup->writebacks;
  cp->invalidations = cp->backup->invalidations;
  cp->mshr_hits = cp->backup->mshr_hits;
  cp->mshr_misses = cp->backup->mshr_misses;
  cp->mshr_full = cp->backup->mshr_full;
  cp->mshr_target_full = cp->backup->mshr_target_full;
  cp->wrong_path_misses =  cp->backup->wrong_path_misses;
  cp->wrong_path_hits =  cp->backup->wrong_path_hits;
  cp->wrong_path_writebacks =  cp->backup->wrong_path_writebacks ;
  cp->wrong_path_replacements =  cp->backup->wrong_path_replacements;
  cp->store_misses =  cp->backup->store_misses;
}
...
void cache_mark_everything_ready(struct cache_t * cp, tick_t now) {
	...
	for (i = 0; i < cp->nsets; i++)
	  for (blk = cp->sets[i].way_head; blk; blk = blk->way_next) {
	    blk->ready = now; /* Mark all blocks as ready right now */
	  }
	/* Mark the main memory bus as ready right now */
	cp->bus_free = now;
  cp->last_tagset = 0;
