...
counter_t sim_pop_insn = 0;

/* Paired-sample comparison support */
FILE * paired_out_file = NULL;


/* Set to non-zero when we are running a job to determine checkpoint library
   size */
struct stat_stat_t ** library_sizing_stat = NULL;
double size_library_epsilon = 0.0;
double size_library_Z = 0.0;
counter_t sampling_munit = 0;
...
banner(FILE *fd, int argc, char **argv)
{
  ...
  fprintf(fd,
	  ...
	  "SMARTSim sampling support Copyright (c) 2003 Carnegie Mellon University.  All Rights Reserved.\n"
...
sim_print_stats(FILE *fd)		/* output stream */
{
  ...
  times(&timeBuffer);
  totalTime = timeBuffer.tms_utime + timeBuffer.tms_stime;
  ...
  if (library_sizing_stat != NULL) {
    struct stat_stat_t * stat = (*library_sizing_stat);
    fprintf(fd, "Determined checkpoint library size for %s @ Z=%2.2f pctError=%2.2f\n", stat->name, size_library_Z, size_library_epsilon);

    if (stat->variant.for_mean.count > 1) {
      double stdev = sqrt( stat->variant.for_mean.sumSq / ( stat->variant.for_mean.count - 1 ) );
      double sqrtn_tuned = stat->variant.for_mean.Z * stdev / stat->variant.for_mean.mean / (stat->variant.for_mean.targetErrorPct / 100.0);
      double ci = stat->variant.for_mean.Z * stdev / stat->variant.for_mean.mean / sqrt( stat->variant.for_mean.count ) * 100.0;
      counter_t k;
      int n_tuned = sqrtn_tuned * sqrtn_tuned;
      k = sim_pop_insn / n_tuned / sampling_munit;
      fprintf(fd, "Library Size: %d  Corresponding k: %lld  Unit size: %lld\n", n_tuned, k, sampling_munit);
      if ( ci > 10.0) {
        fprintf(fd, "Note: confidence interval of +/- %2.2f%% on %s is wide, so estimated library size may be inaccurate.\n", ci, stat->name);
      }
    }
...
exit_now(int exit_code)
{
  ...
  if (simulator_state == WARMING || simulator_state == DRAINING || simulator_state == FAST_FORWARDING) {
     simoo_restore_stats(0);
  }
