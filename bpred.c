...
extern int msmt_pass;
extern int bogus_wrongpath;
             ...
             unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPred2Level:
    pred->dirpred.twolev =
      bpred_dir_create(klass, l1size, l2size, shift_width, xor);

    break;

  case BPred2bit:
    pred->dirpred.bimod =
      bpred_dir_create(klass, bimod_size, 0, 0, 0);
  ...
  unsigned int xor)             /* history xor address flag */
{
  ...
  default:
    panic("bogus branch direction predictor klass");
  }
             ...
             FILE *stream)              /* output stream */
{
  ...
  default:
    panic("bogus branch predictor klass");
  }
}
                ...
                struct stat_sdb_t *sdb) /* stats database */
{
  ...
  stat_reg_counter(sdb, buf, "total number of bpred lookups",
                   &pred->lookups, 0, NULL);
  sprintf(buf, "%s.wplookups", name);
  stat_reg_counter(sdb, buf, "total number of bpred wp lookups",
                   &pred->wplookups, 0, NULL);
                                         ...
                                         * used on mispredict recovery */
{
  ...
  if (pred->retstack.size)
   ...
   if (pred->klass == BPred2Level || pred->klass == BPredComb ) {
             ...
             struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (pred->klass == BPredNotTaken || pred->klass == BPredTaken)
    return;
  ...
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->klass == BPred2Level || pred->klass == BPredComb))
    {
        if (pred_taken != taken) {
            int l1index;
            l1index = (baddr >> MD_BR_SHIFT) & (pred->dirpred.twolev->config.two.l1size - 1);
            dir_update_ptr->recovery_shiftregs[l1index] = dir_update_ptr->recovery_shiftregs[l1index] ^ 1;
         }
   }
...
void bpred_alloc_stats(struct bpred_t *bp) {
  bp->backup = calloc(1, sizeof(struct bpred_stats_t));
}
...
void bpred_backup_stats(struct bpred_t *bp) {
  ...
  bp->backup->wplookups = bp->wplookups;
...
void bpred_restore_stats(struct bpred_t *bp) {
  ...
  bp->wplookups = bp->backup->wplookups;
    ...
    int is_call)                /* non-zero if inst is fn call */
{
    ...
    if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
    {
        ...
        part of the index, take the lower order address bits for the
        other part of the index, rather than the higher order ones */
        l2index = (((l2index ^ (baddr >> MD_BR_SHIFT)) & twolev_dir->config.two.shift_mask) | ((baddr >> MD_BR_SHIFT) << twolev_dir->config.two.shift_width));
        l2index = l2index & (twolev_dir->config.two.l2size - 1);

        /* get a pointer to prediction state information */
        twolev = &twolev_dir->config.two.l2table[l2index];
        meta = &pred->dirpred.meta->config.bimod.table[BIMOD_HASH(pred->dirpred.meta, baddr)];
        bimod = &pred->dirpred.bimod ->config.bimod.table[BIMOD_HASH(pred->dirpred.bimod, baddr)];
        ...
        * MRU and LRU items */
        for (i = index; i < (index+pred->btb.assoc) ; i++) {
            if (pred->btb.btb_data[i].addr == baddr) {
                /* match */
                if (pred->btb.btb_data[i].prev == NULL) {
                    pred->btb.btb_data[i].target = btarget;
                    return;
                }
                pbtb = &pred->btb.btb_data[i];
                hit = 1;
