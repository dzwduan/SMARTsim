...
/* Parameters for determining checkpoint library size */
/*====================================================*/
static char *size_library_opt;
extern struct stat_stat_t **library_sizing_stat;
...
"latency of all pipeline operations.\n"
  ...
  /* sampling options */
  opt_reg_string(odb, "-estimate_k",
		 "Determine sample size and k automatically from provided benchmark length.",
                 &size_library_opt, /* default */NULL,
                 /* print */TRUE, /* format */NULL);

  opt_reg_note(odb,
"  To find the appropiate k value for checkpoint creation:\n"
"\n"
"    -estimate_k <B-instr>:<Z-score>:<percent error>\n"
"\n"
"    <B-instr>   - approximate number of instructions in the benchmark, in billions\n"
"    <Z-score>   - Z-score for the desired confidence level (99.7% confidence == 3.0)\n"
"    <pct-error> - desired confidence interval, in percent (3% == 3.0)\n"
"\n"
"    Example:    -estimate_k 101:3.0:3.0\n"
	       );
...
# endif //(! CMU_AGGRESSIVE_CODE_ELIMINATION )
  ...
  opt_reg_note(odb,
        "Impetus-version-tag:   " CMU_IMPETUS_VERSION_TAG "\n"
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  /* Set sampling parameters to determine checkpoint library size */
  if (size_library_opt) {
    int bmark_len = 0;
    if (sscanf(size_library_opt, "%d:%lf:%lf",
	&bmark_len, &size_library_Z, &size_library_epsilon) != 3)
      fatal("bad size_library parms: <B-instr>:<Z-score>:<pct-error>");

    fprintf(stderr, "Setting sampling parameters to determine checkpoint library size for %d billion instruction benchmark.", bmark_len);
    /* set sampling parameters for a 8000-unit sample.  This typically gives
       a good estimate of coefficient of variation for SPEC2K benchmarks */
    sampling_k = bmark_len * 1000000000LL / sampling_munit / 8000;
    sampling_enabled = TRUE;
    sampling_allwarm = TRUE;
    fastfwd_count = 0;

    /* This can be changed to point to a sc_mean other than CPI if you want to
       size checkpoint libraries on something other than CPI */
    library_sizing_stat = &sample_CPI_mean;
  }
  ...
  if (sampling_enabled && sampling_allwarm) {
...
# endif (! CMU_AGGRESSIVE_CODE_ELIMINATION )
  ...
  sample_CPI_mean = stat_reg_mean(sdb, "sample_CPI_mean",
		   ...
		   /* Z */ k997_Z, /* desiredErrorPct */ 3.0, /* format */NULL);
...
#if 0
  stat_reg_double(sdb, "cycle_sample_stdev",
		   "Standard Deviation of Cycles per Sampling Unit",
		   &standardDeviation, standardDeviation, /* format */NULL);
  stat_reg_double(sdb, "CPI_conf_interval",
		   "99.7% Confidence +/- % CPI error ",
		   &percentageErrorInterval , percentageErrorInterval , NULL);
  stat_reg_int(sdb, "CPI_recommended_k",
		   "k which is likely to achieve 99.7% +/- 3% CPI error",
		   &recommendedK, recommendedK, NULL);
#endif

# if CMU_WATTCH_ENABLED
    stat_reg_formula(sdb, "avg_pwr_insn", "avg pwr per inst. Same as avg_total_power_insn_cc3.","(rename_power_cc3 + bpred_power_cc3 + lsq_power_cc3 + window_power_cc3 + regfile_power_cc3 + icache_power_cc3 + resultbus_power_cc3 + clock_power_cc3 + alu_power_cc3 + dcache_power_cc3 + dcache2_power_cc3)/sim_meas_insn", NULL);
         ...
         (                                                                                                                                   \
            ( (last_dl1_tag == ((addr) & dl1_tagset_mask)) && (((cmd) == Read) || last_dl1_block_dirty ) ) ?                                 \
               (                                                                                                                             \
                  ...
                  ( dl1_extremely_fast_access((cmd), (addr), sim_cycle) ?                                                                    \
                     (                                                                                                                       \
                        NULL                                                                                                                 \
                      ) : (                                                                                                                  \
                        l2_fast_access((cmd), (addr), sim_cycle)                                                                             \
                      )                                                                                                                      \
                   ),                                                                                                                        \
                  ( last_dtlb_tag != ((addr) & dtlb_tagset_mask) ?                                                                           \
                    (                                                                                                                        \
                       last_dtlb_tag = ((addr) & dtlb_tagset_mask),                                                                          \
                       dtlb_extremely_fast_access(dtlb, (addr), sim_cycle)                                                                   \
                     ) : (                                                                                                                   \
                       NULL                                                                                                                  \
                     )                                                                                                                       \
                   )                                                                                                                         \
      ...
      for (icount_high = 0; icount_high <= icount_high_limit; ++icount_high) {
        ...
        do {
          ...
          if (sampling_allwarm && last_il1_tag != (regs.regs_PC & il1_tagset_mask)) {
              ...
              if (! icache_extremely_fast_access(cache_il1, regs.regs_PC, sim_cycle) ) {
                 l2_fast_access(Read, regs.regs_PC, sim_cycle);
              }
              if ( last_itlb_tag != (regs.regs_PC & itlb_tagset_mask)) {
                  last_itlb_tag = (regs.regs_PC & itlb_tagset_mask);
                  icache_extremely_fast_access(itlb, regs.regs_PC, sim_cycle);
              }
          }
